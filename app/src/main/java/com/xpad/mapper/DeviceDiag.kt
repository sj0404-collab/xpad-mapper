...
object DeviceDiag {
    ...
    fun diagnose(device: InputDevice): DiagResult {
        ...
        if (hasGamepad && hasJoystick) {
            ...
            verdict = "Android видит устройство как Bluetooth-клавиатуру, а НЕ как геймпад."
            tip = "Типичная беда беспроводных геймпадов: даже в рeжиме «Gamepad» они подключаются по HID как клавиатура, и игры их не видят.\nРешение:\n• Зажми MODE/Home 5–10 сек — обычно геймпад переподключается в игровой HID-профиль.\n• Либо комбинация переключения режима (Start+вниз / Start+крестовина, зависит от модели).\n• Или подключи по USB-кабелю — по проводу он почти всегда поднимает правильный gamepad-профиль.\n• В этом приложении перейди в «Тест» и выбери устройство — если кнопки прилетают как клавиши клавиатуры, увидишь их в логе."
        } else if (hasDpad) {
            badge = "D-pad"
            color = 0xFFEF6C00.toInt()
            verdict = "Устройство рекламирует только крестовину."
            tip = "Переключите режим контроллера и нажмите «Обновить»."
        } else {
            badge = "Другое"
            color = 0xFFD32F2F.toInt()
            verdict = "Устройство не похоже на игровой контроллер для Android."
            tip = "Попробуйте другой режим геймпада (Gamepad-HID / Xinput). Некоторые BT-геймпады в режиме Dinput не рекламируют SOURCE_GAMEPAD."
        }
